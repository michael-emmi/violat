...
function formatHarness(code, method) {
  let actors =
    ...
    .map(a =>
      ...
      .map(line => line.replace(method,'$\\diamond$'))
...
function formatRow(data) {
  return `${data.method}
    & ${data.invocations}, ${data.sequences}, ${data.values}
...
function formatHeader(className, firstInTable) {
  if (firstInTable)
    return `\\multicolumn{7}{c}{${className}} \\\\
      & \\multicolumn{2}{c}{enumeration} & \multicolumn{3}{c}{failure} & \\\\
      \\cmidrule(lr){2-3} \\cmidrule(lr){4-6}
      method & \\#I, \\#S, \\#V & exp / gen & harness & outcome & frequency & time \\\\
      \\cmidrule(lr){1-7}`.replace(/^\s+/mg,'');
  else
    return `\\midrule
      \\multicolumn{7}{c}{${className}} \\\\
      \\cmidrule(lr){1-7}`.replace(/^\s+/mg,'');
}

function frontMatter() {
  return `\\begin{tabular}{lccllrr}
    \\toprule`.replace(/^\s+/mg,'');
}
