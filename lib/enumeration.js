...
function invocation(spec) {
  return spec
    ? { method: {name: spec.name, parameters: spec.parameters}, arguments: spec.parameters, void: spec.void, atomic: !!spec.trusted }
    : { method: null, arguments: null, void: null, atomic: null }
}
...
function isIntAssignable(type) {
  return type == "int" || type == "java.lang.Integer" || type == "java.lang.Object";
}
...
function valuesOf(type, numValues) {
  ...
  if (isIntAssignable(type))
    ...
    return crossProduct(values, values);

  else if (type && Object.keys(type).length == 1 && isIntAssignable(type.int))
    return crossProduct(values, values)
      .filter(([k1,k2]) => k1 != k2)
...
function placeArgumentValues(numValues) {
  return function*(schema) {
    var types = select('sequences.*.invocations.*.parameters.*')(schema);

    for (var assignment of combine(types, t => valuesOf(t.item, numValues)))
