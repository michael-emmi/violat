...
function valuesOf(type, numValues) {
  ...
  if (type === "int")
    ...
    return values.slice(1).reduce((a,_) => crossProduct(values,a).map(flatten), values);
...
function enumerate(spec, method, numValues, sequences, invocations) {
  return [
    seeds(),
    ...
    placeClass(spec),
    placeConstructor(spec, numValues),
