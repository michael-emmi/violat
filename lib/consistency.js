...
class RelationConsistencyLevel {
  ...
  satisfiesHead(rel, i1, i2) {
    let result = relationIncludes(rel, i1, i2);
    ...
    OUTER_LOOP: for (let i2 of base.values()) {
      ...
      switch (this.comp) {
        case COMPOSITIONS.simple:
          ...
          break;
        case COMPOSITIONS.left:
          let bb1 = base.before(i2);
          invocations = [...new Set(bb1.concat(...bb1.map(i => rel.before(i))))];
          break;
        case COMPOSITIONS.right:
          let bb2 = rel.before(i2);
          invocations = [...new Set(bb2.concat(...bb2.map(i => base.before(i))))];
          break;
      }
