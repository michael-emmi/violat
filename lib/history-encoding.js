...
class HistoryEncoding {
    ...
    let encoding = sequence.invocations.map((inv,idx) => {
      return [0,1].map(j => {
        return this.schema.sequences
          .filter(s => s !== sequence)
          .map((s,i) => `"${s.index}@" + ${exprs.counter(2*idx+j, i)}`)
          .join(` + ":" + `)
  ...
  decode(string) {
      ...
      for (let i of indices) {
        ...
        if (pcs && indices.every(j => j === i || pcs[j] <= counters[j])) {
          ...
          let event = new Event({
            ...
            sid: i,
            invocation: this.schema.sequences.find(s => s.index == i).invocations[invIdx],
            value: isCall ? undefined : pcs.result
          });
    ...
    let history = new Trace(this.schema, events);
  ...
  _parse(string) {
    ...
    string.split(", ").forEach((resultStr, resultIdx) => {
      let thisSeqIdx = this.schema.sequences[resultIdx].index;
