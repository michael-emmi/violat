...
${schema.outcomes.map(outcome => {
    ...
    let sequence = outcome.sequence.filter((_,i) => resultIdxs.includes(i));
    let escaped = escape(sequence.join(', '));
    let expect = 'Expect.ACCEPTABLE';
    if (outcome.properties.length > 0)
      expect += "_INTERESTING";
    let desc = outcome.properties.join(",");

    return `@Outcome(id = "${escaped}", expect = ${expect}, desc = "${desc}")`;
