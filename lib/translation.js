...
module.exports = function(schema, id) {
  let testName = `${schema.class.split('.').pop()}${id}Test${schema.id || '0'}`
  debug(`translating schema: %s`, schema);
  ...
  let hasResult = (seq, inv) => seq.index != initial && !inv.method.void;
...
${schema.outcomes.map(outcome => {
    ...
    let id = escape(outcome.values().filter((_,i) => resultIdxs.includes(i))
      .join(', '));

    let expect = outcome.isWeak()
        ? "Expect.ACCEPTABLE_INTERESTING"
        : "Expect.ACCEPTABLE";

    let desc = [].concat(outcome.consistency)
...
public class ${testName} {
    ${className} obj = new ${className}(${(schema.arguments || []).join(", ")});
