...
class JCStressCodeGenerator extends JavaCodeGenerator {
  ...
  postInvocation(sequence, invocation) {
    ...
    return [
      ...(p !== undefined ? [`result.r${p+1} = ${this.resultAdapter()}(r);`] : [])
    ];
  }
...
class JCStressPCBasedHistoryRecordingCodeGenerator extends JCStressCodeGenerator {
  constructor(schema, id) {
    super(schema, id);
  }
  ...
  postInvocation(sequence, invocation) {
    ...
    return [
      ...(p !== undefined ? [`r${pos} = ${this.resultAdapter()}(r);`] : [])
    ];
  }
  ...
  endSequence(sequence) {
    return [
      '',
      `result.r${sequence.index} = ${this.encodeHistoryLog(sequence)};`
    ];
  }

  encodeHistoryLog(sequence) {
    return sequence.invocations.map((inv,idx) => {
      return [0,1].map(j => {
        return this.schema.sequences
          .filter(s => s !== sequence)
          .map((s,i) => `"${s.index}@" + pcs[${2*idx+j}][${i}]`)
          .join(` + ":" + `)
      }).join(` + ";" + `);
    }).join(` + ";" + `);
  }
