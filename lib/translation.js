...
class JCStressHistoryRecordingCodeGenerator extends JCStressCodeGenerator {
  ...
  fieldDecls() {
    return [
      ...super.fieldDecls(),
      ...this.schema.sequences.map(s => `NoReorderCounter pc${s.index} = new NoReorderCounter();`),
    ];
  }
  ...
  beforeInvocation(sequence, invocation) {
    return [
      ...this.schema.sequences.filter(s => s != sequence).map((s,i) => `pcs[n][${i}] = pc${s.index}.get();`),
      `n++;`,
      `pc${sequence.index}.increment();`,
  ...
  endSequence(sequence) {
    ...
    return [
      '',
      `result.r${sequence.index+1} = ${encoding};`
    ];
  }
}
