    ...
    for (let predecessor of predecessors) {
      ...
      while (workList.length) {
        ...
        if (args.weak && !(invocation.atomic && predecessor.atomic)) {
          ...
          let properties = Properties.unit('weak-visibility');

          if (programOrder.isBefore(predecessor, invocation))
            properties.add('relaxed-visibility');

          visibilities.push(viz.extend([], properties.join(linearization.properties)));
        }

        debug(`considering visible predecessor`);
        visibilities.push(viz.extend([[predecessor, invocation]], linearization.properties));
      }
    }
