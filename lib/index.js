...
async function schemaFile(specFile, method, numValues, sequences, invocations) {
  let spec = JSON.parse(fs.readFileSync(specFile, 'utf8'));
  let dstFile = path.join(
    config.outputPath,
    'schemas',
    path.basename(specFile)
      .replace('.json', `.${method}.${numValues}v.${sequences}s.${invocations}i.json`)
  );
  mkdirp.sync(path.dirname(dstFile));
  let ary = Array.from(enumeration.generator(spec, method, numValues, sequences, invocations)());
...
async function testMethod(specFile, method, numValues, sequences, invocations) {
  try {
    let chunkSize = 100;

    let schemas = await schemaFile(specFile, method, numValues, sequences, invocations);
    ...
    for (let idx in chunks) {
      ...
      if (result.failedHarness) {
        let dstFile = path.join(
          ...
          `${result.failedHarness.replace(/[.]/,'/')}.java`);
...
async function testUntrustedMethods(specFile, numValues, sequences, invocations) {
  ...
  for (let m of methods) {
    ...
    let result = await testMethod(specFile, m.name, numValues, sequences, invocations);
    if (result.failedHarness) {
