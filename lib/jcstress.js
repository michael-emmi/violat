...
class History {
  ...
  static decode(encoding, schema) {
    ...
    // add additional ordering constarints.
    let sequences = encoding.split(", ").map(seq => {
      let pcs = seq.split(';').map(entry => {
        let [seqIdx, pc] = entry.split(':');
        let invIdx = pc / 2 - 1;
        //
        let invocation = schema.sequences.find(s => s.index == seqIdx)[invIdx];
      });
    });
...
class JCStressHistoryGenerator extends JCStressRunner {
  ...
  _resultData(result) {
    return {
      histories: result.outcomes.map(o => History.decode(o.value))
    };
  }
}
