...
function copyTemplate(dstPath) {
  return new Promise((resolve, reject) => {
    let templateDir = path.join(__dirname, '../resources/jcstress');
...
function test(harnesses, onTick) {
  return new Promise(async (resolve, reject) => {
    ...
    let workPath = path.resolve('tests');
    ...
    for (let harness of harnesses) {
      let dstFile = path.resolve(testsPath(workPath), harness.relativePath);
      ...
      if (name) {
        ...
        if (!lastFailedName)
          ...
          resolve({
            status: 'fail',
            name: lastFailedName,
            harness: await getHarness(workPath, lastFailedName),
            values: result
          });
