...
class Event {
  ...
  toString() {
    return `${this.kind}[${this.id}] ${this.kind === 'call' ? this.invocation : ''}`;
  }
}
...
class History {
  ...
  static decode(string, schema) {
    debug(`decoding record %s for schema %s`, string, schema);
    let pairs = [];

    // add observed ordering constarints.
    string.split(", ").forEach((resultStr, resultIdx) => {
      resultStr.split(';').forEach((stepStr, stepIdx) => {
        let invocation = schema.sequences[resultIdx].invocations[stepIdx];

        stepStr.split(':').forEach((entryStr, entryIdx) => {
          let [thatSeqIdx, thatSeqPc] = entryStr.split('@');
          let thatInvIdx = Math.floor(thatSeqPc/2) - 1;
          if (thatInvIdx >= 0) {
            let predecessor = schema.sequences.find(s => s.index == thatSeqIdx).invocations[thatInvIdx];

            debug(`decoded: %s < %s`, predecessor, invocation);
            pairs.push([predecessor, invocation]);
          }
        });
      });
    });

    let order = schema.getProgramOrder();
    pairs.forEach(([i,j]) => order.sequence(i,j));
    return new History(order);
  }

  static _decode_1(string, schema) {
    let sequences = {};
    string.split(", ").forEach((resultStr, resultIdx) => {
      let thisSeqIdx = schema.sequences[resultIdx].index;
    ...
    detail(`decoded %s to %o`, string, sequences);
    return sequences;
  }

  static _history_1(string, schema) {
    let sequences = History._decode(string, schema);
    let indices = Object.keys(sequences);
    let counters = indices.reduce((cs,i) => Object.assign({}, cs, {[i]: 0}), {});
    let events = [];

    detail(`generating events for schema %j`, schema);
    ...
    while (indices.some(i => counters[i] < Object.keys(sequences[i]).length)) {
      ...
      for (let i of indices) {
        ...
        if (pcs && indices.every(j => j === i || pcs[j] <= counters[j])) {
          ...
          let event = new Event({
            kind: counters[i] % 2 === 0 ? 'call' : 'return',
            id: `${invIdx}@${i}`,
            invocation: schema.sequences.find(s => s.index == i).invocations[invIdx]
          });
          trace(`got event: %o`, event);
          events.push(event);
          counters[i]++;
          break;
        }
  ...
  static _history(string, schema) {
    ...
    while (indices.some(i => counters[i] < Object.keys(sequences[i]).length)) {
      detail(`counter: %o`, counters);

      let min = Math.min(...Object.keys(sequences)
        .map(j => sequences[j][counters[j]])
        .filter(Number.isInteger));

      detail(`minimum time: %s`, min);

      let i = Object.keys(sequences).find(j => sequences[j][counters[j]] <= min);

      detail(`next sequence: %s`, i);
  ...
  static _decode(string, schema) {
    ...
    string.split(", ").forEach((resultStr, resultIdx) => {
      ...
      resultStr.split(';').forEach((stepStr, stepIdx) => {
        sequences[thisSeqIdx][stepIdx] = +stepStr;
      });
    });
    detail(`decoded %s to %o`, string, sequences);
    return sequences;
...
class JCStressHistoryGenerator extends JCStressRunner {
  ...
  static *_codeGenerator(schemas, id) {
    for (let schema of schemas)
      yield new JCStressHistoryRecordingCodeGenerator(schema, id).toString();
  }

  _resultData(result) {
    let schema = this.schemas.find(s => s.id === result.index);
    return {
      histories: result.outcomes.map(o => History._history(o.value, schema))
    };
  }
}
