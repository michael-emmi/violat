...
class JCStressTester {
  constructor(schemas, testName, maxViolations = 1) {
    ...
    this.initialized = this._initialize(testName);
  }
  ...
  _initialize(testName) {
    return new Promise(async (resolve, reject) => {
      ...
      for (let schema of this.schemas) {
        let harness = await translate(schema, testName);
  ...
  _result(status, name, idx) {
    let schema = this.schemas.find(s => s.id === idx);
    detail(`got result %s for test %s from schema %s`, status, name, schema);
    this.results.push({
      name: name,
      schema: schema,
      status: status === 'OK',
  ...
  _outcome(value, count, expectation, description) {
    ...
    let outcome = getOutcome(value, last.schema).observe(count);
    detail(`got outcome: %s`, outcome);
    assert.ok(!last.status || outcome.consistency);
