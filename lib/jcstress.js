...
function matchResult(result, predictedResults, schema) {
  ...
  for (let seq of schema.sequences) {
    for (let inv of seq.invocations) {
      ...
      if (pos && observedResults[pos] !== predictedResults[idx]) {
        return false;
      }
    }
  }
...
function getConsistency(result, schema) {
...
module.exports = function(schemas, testName, onTick, maxViolations = 1) {
  return new Promise(async (resolve, reject) => {
    ...
    lines.on('data', async data => {
      ...
      let [status, name, idx] = get(/\[(OK|FAILED)\]\s+([A-Za-z_$.]+([0-9]+))\s*$/)(data);
      if (status) {
        results.push({
          name: name,
          schema: schemas[idx],
      ...
      let [result, count, expectation, description] =
      ...
      if (result) {
        ...
        last.outcomes.push({
          result: result,
          consistency: getConsistency(result, last.schema),
