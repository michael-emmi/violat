...
const { RELATIONS, COMPARISONS, BASIC_LEVELS, Consistency } = require('./consistency');
        ...
        if (outcome.results[idx] !== cummulativeOutcome.results[idx]) {
          ...
          cummulativeOutcome.consistency =
            cummulativeOutcome.consistency.weakenSimpleLevel(
              BASIC_LEVELS.consistentreturns, conflict);
        }
      }
    }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let schema of schemas) {
      ...
      for (let outcome of await getOutcomes(executor, schema, args)) {
        let consistency = outcome.consistency.map(c => c.simplify());
        let atomic = consistency.join("") === Consistency.full().simplify().toString();
        schema.outcomes.push({
          values: Object.values(outcome.results),
          expected: atomic ? true : undefined,
          description: atomic ? ["atomic"] : consistency
        });
      }
    }
