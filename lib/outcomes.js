...
async function getOutcomes(executor, schema, args) {
  ...
  debugdetail(`using happens before %s`, programOrder);

  for (let lin of linearization(programOrder, args)) {
    debugdetail(`using linearization %s`, lin);

    for (let viz of visibility(programOrder, lin, args)) {
      debugdetail(`using visibility %s`, viz);
  ...
  if (debugdetail.enabled) {
    for (let outcome of outcomes)
      debugdetail(`  %s`, outcome);
  }
...
async function getOutcome(executor, schema, linearization, visibility, args) {
  ...
  debugdetail(`got outcome: %s`, outcome);
  return outcome;
}
...
async function getResults(executor, schema, invocations) {
  ...
  let returns =
    executorCache[schema.class][invocations]
    || await executor.query(getQuerySequence(schema, invocations));
