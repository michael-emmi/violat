...
function splitSchema(schema, spec) {
  let result = JSON.parse(JSON.stringify(schema));
  for (let sequence of result.sequences) {
    for (let invocation of sequence.invocations) {
      ...
      if (method.split) {
        ...
        let repl = spec.methods.find(m => m.name === method.split);

        if (Array.isArray(collection)) {
          sequence.invocations.splice(index, 1, ...collection.map(x => ({
            method: repl,
            arguments: [x],
            atomic: true
          })));
          ...
          sequence.invocations.splice(index, 1, ...Object.entries(collection).map(([k,v]) => ({
            method: repl,
            arguments: [k,v],
            atomic: true
          })));
        }
        debug(`after split: %O`, result);
      }
    }
  }
  return result;
...
async function getOutcomes(executor, schema, args) {
  ...
  let unique = Outcome.merged_minimals(outcomes);
...
async function getOutcome(executor, schema, linearization, visibility, args) {
  ...
  for (let invocation of linearization.sequence) {
    ...
    if (!args.weakRelaxReturns
        && cummulativeOutcome.properties.get().includes('R'))
      return undefined;
  }
