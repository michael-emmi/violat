...
class PartialOrder {
  ...
  static from(iterable) {
    let po = new PartialOrder();
    let last;
    for (let item of iterable) {
      po.add(item);
      if (last)
        po.sequence(last, item);
      last = item;
    }
    return po;
  }
  ...
  toString() {
    return `{ ${[...this.basis.entries()].map(([n,preds]) => `${n} > {${[...preds].join(', ')}}`).join('; ')} }`;
  }
  ...
  linearizations() {
    ...
    while (workList.length) {
      ...
      debug(`partial linearization %O with remainder %O`, seq, po);
      if (po.values().length)
